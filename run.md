...
> Paste this into Claude Code to start a new analysis session.
> Or just type: "run a market advisor session" — Claude Code will read CLAUDE.md automatically.
...
- **Add a specific question** in the optional context field. Broad sessions are good
  ...
  bond ETF") produce deeper teaching moments.

- **Read the session log after** — the `sessions/` folder is your investment journal.
  Re-reading past sessions before starting a new one builds continuity.
...
- **Push back on Claude** — if a scenario or explanation doesn't make sense, ask
  ...
  designed to go deeper, not to give you a polished but shallow answer.
